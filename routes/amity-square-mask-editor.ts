...
export default function AmitySquareMaskEditor() {
  ...
  // ── Load mask PNG and build RGBA data ─────────────────────────────────
  const loadMask = (cfg: AmityConfig) => {
    ...
    img.onload = () => {
      ...
      const raw = new Uint8ClampedArray(
        mc.getContext("2d")!.getImageData(0, 0, mc.width, mc.height).data
      );
  ...
  // ── Render canvas from current maskData ────────────────────────────────
  ...
  // ── Publish: POST mask PNG + objects JSON to workspace ───────────────
  ...
  // ── Render loop for objects changes ───────────────────────────────────
    ...
    <div ref={containerRef} className="relative min-h-screen overflow-hidden bg-[#0b0f0b]" style={{ background: "radial-gradient(ellipse at 50% 40%,#111d11 0%,#080c08 100%)" }}>
      ...
      {mode === "paint" && (
        <div className="absolute right-4 top-14 z-10 flex flex-col gap-2 rounded-xl border border-white/10 bg-black/75 p-3 text-xs text-white backdrop-blur">
          ...
          <button onClick={() => setBrush(0)} className={`flex items-center gap-2 rounded px-3 py-1.5 ${brush === 0 ? "bg-emerald-700 ring-1 ring-emerald-400" : "bg-black/60 hover:bg-black/80"}`}>
            <span className="h-4 w-4 rounded" style={{ background: "#00CC50" }} /> Walkable
          </button>
          <button onClick={() => setBrush(1)} className={`flex items-center gap-2 rounded px-3 py-1.5 ${brush === 1 ? "bg-red-800 ring-1 ring-red-400" : "bg-black/60 hover:bg-black/80"}`}>
      ...
      {mode === "objects" && (
        <div className="absolute right-4 top-14 z-10 flex flex-col gap-2 rounded-xl border border-white/10 bg-black/75 p-3 text-xs text-white backdrop-blur max-h-[70vh] overflow-y-auto">
          ...
          <select value={objType} onChange={e => setObjType(e.target.value)} className="rounded bg-black/60 px-2 py-1 text-white/80">
            {config && Object.entries(config.objectTypes).map(([k, v]) => (
          ...
          <button onClick={() => {
            ...
            setObjects(prev => [...prev, { id: `obj-${Date.now()}`, x: 503, y: 387, label: objLabel.trim(), dialog: objDialog.split("\n").filter(l => l.trim()), type: objType }]);
            setObjLabel(""); setObjDialog("");
          }} className="rounded bg-emerald-800 px-3 py-1.5 text-emerald-100 hover:bg-emerald-700">Add</button>
